...
//pheromone params
uniform float decayWeight;			//how much each pixels pheromones decay (multiplied)
uniform float diffusionWeight;		//mixing value to control how much is diffused vs kept
...
vec2 updatePos(Particle particle) {
	return particle.pos + maxSpeed*vec2(cos(particle.heading),sin(particle.heading));
}
